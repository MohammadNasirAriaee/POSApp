                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td class="py-4 px-6">
                                        <form action="{{ route('employees.toggle-status', $employee) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" title="Click to cycle status" aria-label="Change status for {{ $employee->name }}, currently {{ $employee->status_label }}" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ring-inset {{ $employee->status_badge_class }} hover:opacity-80 transition-opacity">
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('employees.show', $employee) }}" class="p-1.5 rounded-lg text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-all" title="View Profile" aria-label="View profile for {{ $employee->name }}">
                                                <svg class="w-4 h-4" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <a href="{{ route('employees.edit', $employee) }}" class="p-1.5 rounded-lg text-slate-500 hover:text-amber-600 hover:bg-amber-50 transition-all" title="Edit Employee" aria-label="Edit {{ $employee->name }}">
                                                <svg class="w-4 h-4" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <form action="{{ route('employees.destroy', $employee) }}" method="POST" class="inline" onsubmit="return confirm('Delete {{ addslashes($employee->name) }}?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all" title="Delete Employee" aria-label="Delete {{ $employee->name }}">
                                                    <svg class="w-4 h-4" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24">
