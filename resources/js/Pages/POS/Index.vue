...
const filteredProducts = computed(() => {
    return props.products.filter((product) => {
        ...
        const matchesCategory =
            ...
            String(product.category_id) === String(activeCategory.value);
...
const processCheckout = () => {
    ...
    checkoutForm.cart = cart.value.map((i) => ({
        id: i.id,
        quantity: i.quantity,
        price: i.price,
    }));
...
<template>
    <AppLayout>
        <div class="h-[calc(100vh-8rem)] flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 flex flex-col min-h-0 bg-surface-50">
                ...
                <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-4">
                    <div
                        ...
                        class="h-full flex flex-col items-center justify-center text-surface-500 bg-white rounded-xl border border-dashed border-surface-300"
                    >
                        ...
                        >
                            <div class="absolute top-2 right-2">
                                <span
                                    ...
                                    class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide bg-rose-100 text-rose-700"
                                    >Out</span
                                >
                                <span
                                    v-else
                                    class="text-xs font-semibold text-surface-400"
                                    >{{ product.stock_quantity }} left</span
                                >
                            </div>

                            <div
                                class="h-12 w-12 rounded-lg bg-surface-100 text-surface-400 flex items-center justify-center mb-3 group-hover:bg-primary-50 group-hover:text-primary-600 transition-colors"
                            >
                            ...
                            <h3
                                class="font-bold text-surface-900 leading-tight mb-1 truncate"
                            >
                                {{ product.name }}
                            </h3>
            ...
            <!-- Right Side: Cart -->
            <div
                class="w-full lg:w-96 shrink-0 flex flex-col metronic-card h-full max-h-full bg-white"
            >
                <!-- Cart Header -->
                <div
                    class="p-4 border-b border-surface-200 bg-surface-50/50 flex items-center justify-between shrink-0"
                >
                    <div class="flex items-center gap-2">
                        <ShoppingCart class="w-5 h-5 text-primary-600" />
                        <h2 class="font-bold text-surface-900">
                            Current Order
                        </h2>
                    </div>
                    <span
                        class="bg-surface-800 text-white text-xs font-bold px-2 py-1 rounded-md"
                ...
                <div class="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    <div
                        v-if="cart.length === 0"
                        class="h-full flex flex-col items-center justify-center text-surface-400"
                    >
                    ...
                    <div v-else class="space-y-3">
                        ...
                        >
                            <div class="flex-1 min-w-0">
                                <h4
                                    class="font-bold text-surface-900 text-sm truncate"
                                >
                                    {{ item.name }}
                                </h4>
                                <div
                                ...
                                >
                                    <span
                                        class="text-primary-600 font-bold text-sm"
                                        >{{ formatMoney(item.price) }}</span
                                    >
                                    <span
                                        class="text-surface-500 text-xs font-semibold"
                                        >{{
                                            formatMoney(
                            ...
                            <div
                                class="flex items-center gap-1 bg-surface-100 rounded-lg p-1 shrink-0"
                            >
                                <button
                                ...
                                <span
                                    class="w-6 text-center text-sm font-bold text-surface-900"
                                    >{{ item.quantity }}</span
                                >
                                <button
                                    @click="updateQuantity(item, 1)"
                ...
                <div
                    class="p-4 border-t border-surface-200 bg-surface-50/50 shrink-0 space-y-3"
                >
                    ...
                    >
                        <span class="text-surface-900 font-black text-lg"
                            >Total</span
                        >
                        <span
                            class="text-primary-600 font-black text-2xl tracking-tight"
        ...
        >
            ...
            >
                ...
                <div class="p-6 space-y-6">
                    <div
                        class="bg-primary-50 rounded-xl p-4 flex items-center justify-between"
                    >
                    ...
                    <div>
                        ...
                        <div
                            ...
                            "
                            class="mt-2 text-emerald-600 font-semibold text-sm flex justify-between"
                        >
                        ...
                        <div
                            v-else-if="
                                ...
                                parseFloat(tenderedAmount) < cartTotal
                            "
                            class="mt-2 text-rose-600 font-semibold text-sm"
                        >
                ...
                <div
                    class="p-6 border-t border-surface-200 bg-surface-50 flex gap-3"
                >
                    <button
                        @click="showCheckoutModal = false"
                        class="metronic-btn metronic-btn-light flex-1"
                    >
                        Cancel
                    </button>
                    <button
